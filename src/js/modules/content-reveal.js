// content-reveal.js — Scroll reveal con GSAP + ScrollTrigger
// Activacion: autoDetect en site.js via [data-reveal-group]
// Webflow carga window.gsap y window.ScrollTrigger nativamente (3.15.0)
// waitForGSAP necesario por Cloudflare Rocket Loader (nuestro module corre antes que GSAP)
  ...
  return new Promise((resolve, reject) => {
    ...
    const check = setInterval(() => {
      if (window.gsap && window.ScrollTrigger) {
        ...
        clearInterval(check);
        reject(new Error('gsap/ScrollTrigger not found'));
      }
    }, 50);
  });
}

function initContentRevealScroll(gsap) {
...
export function init() {
  waitForGSAP()
    .then(gsap => initContentRevealScroll(gsap))
    .catch(err => console.warn('[atom] content-reveal:', err));
}
